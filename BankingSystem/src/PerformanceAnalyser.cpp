...
void DoPerformanceAnalysis(int nNumThreads, int nNumIterations, double duration_StackWithLock, double duration_StackLockFree)
{
	std::ofstream performanceFile("performanceData.txt", std::ios_base::app);
	static int firstTime = 0;
	if (0 == firstTime)
	{
		performanceFile << center("Thread count", 10) << " | "
			<< center("Iteration count per thread", 10) << " | "
			<< center("Bank Transaction time with Lock", 10) << " | "
			<< center("Bank Transaction time Lock free", 10) << " | "
	...
	performanceFile << prd(nNumThreads, 0, 12) << " | "
		...
		<< prd(duration_StackWithLock, 4, 31) << " | "
		<< prd(duration_StackLockFree, 4, 31) << " | "
